...
class TestRunMistakesPost:
    ...
    @patch("v2.social_weekly.insert_session_record", return_value=1)
    @patch("v2.social_weekly.complete_session")
    @patch("v2.social_weekly.get_session_for_date", return_value=None)
    ...
    def test_skipped_when_no_data(
        self, mock_bs_client, mock_tw_client, mock_ctx,
    ...
    def test_skipped_when_already_posted(
        self, mock_bs_client, mock_tw_client, mock_ctx, mock_dedup,
        mock_get_sess, mock_complete, mock_sess, mock_is_td,
    ):
        from v2.social_weekly import run_mistakes_post
    ...
    @patch("v2.social_weekly.is_trading_day", return_value=True)
    @patch("v2.social_weekly.insert_session_record", return_value=77)
    @patch("v2.social_weekly.complete_session")
    @patch("v2.social_weekly.fail_session")
    @patch("v2.social_weekly.get_session_for_date", return_value=None)
    @patch("v2.social_weekly.insert_tweet", return_value=1)
    @patch("v2.social_weekly.posted_tweet_exists", return_value=False)
    @patch("v2.social_weekly.post_to_bluesky")
    ...
    @patch("v2.social_weekly.get_twitter_client")
    @patch("v2.social_weekly.get_bluesky_client")
    def test_mistakes_creates_session_row_and_threads_id(
        self, mock_bs_client, mock_tw_client, mock_ctx, mock_gen,
        mock_post_tw, mock_post_bs, mock_dedup, mock_insert_tweet,
        mock_get_sess, mock_fail, mock_complete, mock_sess, mock_is_td,
    ):
        from v2.social_weekly import run_mistakes_post
        ...
        run_mistakes_post(today=date(2026, 5, 8))

        args, kwargs = mock_sess.call_args
        assert (
            kwargs.get("session_type") == "weekly_mistakes"
            or (len(args) >= 2 and args[1] == "weekly_mistakes")
        )
...
class TestRunAttributionPost:
    ...
    @patch("v2.social_weekly.is_trading_day", return_value=True)
    @patch("v2.social_weekly.insert_session_record", return_value=99)
    @patch("v2.social_weekly.complete_session")
    @patch("v2.social_weekly.fail_session")
    @patch("v2.social_weekly.get_session_for_date", return_value=None)
    @patch("v2.social_weekly.insert_tweet", return_value=1)
    ...
    @patch("v2.social_weekly.post_to_bluesky")
    @patch("v2.social_weekly.post_tweet")
    @patch("v2.social_weekly.generate_attribution_post")
    @patch("v2.social_weekly.gather_attribution_context", return_value="ctx")
    @patch("v2.social_weekly.get_twitter_client")
    @patch("v2.social_weekly.get_bluesky_client")
    def test_attribution_creates_session_row_and_threads_id(
        self, mock_bs_client, mock_tw_client, mock_ctx, mock_gen,
        mock_post_tw, mock_post_bs, mock_dedup, mock_insert_tweet,
        mock_get_sess, mock_fail, mock_complete, mock_sess, mock_is_td,
    ):
        ...
        mock_tw_client.return_value = object()
        mock_bs_client.return_value = object()
        mock_gen.return_value = {"text": "x", "type": "weekly_attribution"}
        mock_post_tw.return_value = {"posted": True, "tweet_id": "tw1",
                                     "text": "x", "type": "weekly_attribution",
                                     "error": None}
        mock_post_bs.return_value = {"posted": True, "post_id": "bs1",
                                     "text": "x", "type": "weekly_attribution",
                                     "error": None}

        run_attribution_post(today=date(2026, 5, 8))

        args, kwargs = mock_sess.call_args
        assert (
            kwargs.get("session_type") == "weekly_attribution"
            or (len(args) >= 2 and args[1] == "weekly_attribution")
        )
        mock_complete.assert_called_once_with(99)
        mock_fail.assert_not_called()
        assert mock_insert_tweet.call_count == 2
        for call in mock_insert_tweet.call_args_list:
            assert call.kwargs.get("session_id") == 99
